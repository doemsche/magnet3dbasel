...
module.exports = function(grunt) {
  ...
  function filterStatAMigr(source, dest, codes, options, done) {
    var headers = [];
  ...
  grunt.registerMultiTask('filterStatAMigr', 'Filter csv data', function() {
      ...
      if (err) {
        ...
        files.forEach(function(file) {
          file.src.forEach(function(src) {
            ...
            filterStatAMigr(src, file.dest, codes, options, function(err) {
              if (err) {
                grunt.log.error(err);
              } else {
                grunt.log.ok();
